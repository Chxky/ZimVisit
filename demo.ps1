...
if ((docker compose ps -q 2>$null).Length -eq 0) {
    ...
    docker compose up -d operator-dashboard government-portal traveler-portal
}
...
for ($i = 0; $i -lt 30; $i++) {
    try {
        $r = Invoke-WebRequest -Uri "http://localhost:3005/health/liveness" -UseBasicParsing -DisableKeepAlive
...
Write-Host ""
  ...
  Write-Host "  Traveler Portal: http://localhost:3003"
