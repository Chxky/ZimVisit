  ...
  useEffect(() => {
    ...
    const runAudit = async () => {
      try {
        ...
        let data: any;
        try {
          const res = await complianceApi.getLeakage({
            startDate: start.toISOString(),
            endDate: end.toISOString(),
          });
          data = res || { estimatedLeakage: 0, nonCompliantBookings: 0 };
        } catch (e) {
        ...
        const stages = [
          ...
          { progress: 60, msg: `Estimated revenue leakage: $${data.estimatedLeakage}...` },
          {
            progress: 85,
            msg: hasLeakage
              ? 'Anomalies detected in operator tax remittance...'
              : 'Detecting network vulnerabilities & anomalies...',
          },
          {
            progress: 100,
            msg: hasLeakage ? 'Audit complete. ACTION REQUIRED.' : 'Audit complete. Zero threats detected.',
          },
        ];
      ...
      >
        ...
        {isComplete && (
          <div
            style={{
              padding: '16px 24px',
              background: scanStatus.includes('REQUIRED')
                ? 'rgba(239, 68, 68, 0.1)'
                : 'rgba(34, 197, 94, 0.1)',
              borderTop: scanStatus.includes('REQUIRED')
          ...
          >
            <Space>
              {scanStatus.includes('REQUIRED') ? (
                <WarningOutlined style={{ color: '#ef4444' }} />
              ) : (
                <LockOutlined style={{ color: '#22c55e' }} />
              )}
              <Text
                style={{ color: scanStatus.includes('REQUIRED') ? '#ef4444' : '#22c55e', fontWeight: 600 }}
              >
                {scanStatus.includes('REQUIRED') ? 'Compliance Alert Triggered' : 'System Fully Compliant'}
              </Text>
            </Space>
            <Tag
              color={scanStatus.includes('REQUIRED') ? 'error' : 'success'}
