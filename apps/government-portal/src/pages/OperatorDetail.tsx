...
const auditHistory = [
  {
    ...
    auditor: 'T. Moyo',
    notes:
      'BSP reconciliation discrepancies noted. 30-day corrective action period initiated. Levy remittance schedule adjusted.',
    severity: 'medium',
  },
  {
    date: '2025-11-10',
    ...
    auditor: 'R. Ncube',
    notes:
      'Agent velocity anomaly detected. 3 bookings flagged for manual review. BSP reference mismatch confirmed.',
    severity: 'high',
  },
  {
    date: '2025-08-15',
    ...
    auditor: 'P. Dlamini',
    notes:
      'All compliance checks passed. Minor documentation update recommended for agent registration records.',
    severity: 'low',
  },
  {
    date: '2025-03-20',
    ...
    result: 'Pass',
    auditor: 'T. Moyo',
    notes: 'Excellent record keeping. All levy payments on time. BSP connection stable.',
    severity: 'low',
  },
];
...
/* ── Compliance Ring ──────────────────────────────────────── */
const ComplianceRing: React.FC<{ percent: number; size?: number; label?: string }> = ({
  percent,
  size = 120,
  label,
}) => {
...
export const OperatorDetail: React.FC = () => {
  ...
  // Map of operator data by ID for demo purposes
  const operatorsData: Record<string, typeof operator> = {
    'OP-001': operator,
    'OP-002': {
      ...operator,
      id: 'OP-002',
      name: 'Zambezi Helicopters',
      email: 'info@zambeziheli.co.zw',
      complianceRate: 95,
      riskScore: 12,
      totalRevenue: 520000,
      totalLevy: 10400,
    ...
    'OP-003': {
      ...
      email: 'bookings@greatzimtours.co.zw',
      complianceRate: 88,
      riskScore: 22,
      totalRevenue: 280000,
      totalLevy: 5600,
      totalBookings: 340,
      agentCount: 6,
      recentFlags: ['1 late levy remittance'],
    },
  };

  const currentOperator = operatorsData[id || 'OP-001'] || operator;
  ...
  const bookingColumns = [
    {
      ...
      key: 'ref',
      render: (v: string) => (
        <Text strong style={{ fontFamily: "'Inter', monospace", fontSize: 12 }}>
          {v}
        </Text>
      ),
    },
    ...
    },
    {
      ...
      render: (s: string) => {
        ...
        const bg: Record<string, string> = {
          confirmed: 'rgba(5,150,105,0.08)',
          pending: 'rgba(245,158,11,0.08)',
          completed: 'rgba(30,27,75,0.08)',
  ...
  const agentColumns = [
    ...
    },
    {
      ...
      render: (s: string) => {
        ...
        return (
          <Badge
            status={s === 'active' ? 'success' : s === 'flagged' ? 'error' : 'default'}
            text={<Text style={{ fontSize: 12, color: colors[s], fontWeight: 600 }}>{s.toUpperCase()}</Text>}
          />
        );
      },
    },
  ...
  const c = statusConfig[currentOperator.status];
  ...
  return (
    <div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div>
            ...
            <Text style={{ fontSize: 12, color: '#64748b', display: 'block', marginTop: 2 }}>
              {currentOperator.id} | License: {currentOperator.licenseNumber}
            </Text>
          </div>
        </div>
        <Space>
      ...
      {currentOperator.recentFlags.length > 0 && (
          ...
          <Alert
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <Space>
                  ...
                  <Text style={{ fontSize: 13 }}>
                    Recent Compliance Flags ({currentOperator.recentFlags.length})
                  </Text>
                </Space>
            ...
            description={
              <ul style={{ margin: '8px 0 0', paddingLeft: 16 }}>
                {currentOperator.recentFlags.map((f, i) => (
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 20 }}>
        {[
          {
            label: 'Total Revenue',
            value: `$${(currentOperator.totalRevenue / 1000).toFixed(0)}K`,
            icon: <DollarOutlined />,
          ...
          {
            label: 'Levy Remitted',
            value: `$${(currentOperator.totalLevy / 1000).toFixed(1)}K`,
            icon: <BankOutlined />,
          ...
          },
          {
            ...
            value: currentOperator.totalBookings.toString(),
          ...
          {
            label: 'Active Agents',
            value: currentOperator.agentCount.toString(),
            icon: <TeamOutlined />,
            ...
            <div className={`kpi-card animate-fade-in-up stagger-${idx + 1}`}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                  ...
                  <div className="kpi-value" style={{ color: '#1e1b4b', marginTop: 8 }}>
                    {kpi.value}
                  </div>
      ...
      {/* Main Content Tabs */}
      <div
        className="gradient-border-card animate-fade-in-up stagger-4"
          ...
          items={[
                ...
                <Row gutter={[20, 20]} style={{ paddingBottom: 20 }}>
                  ...
                  <Col xs={24} lg={8}>
                    ...
                    >
                      ...
                      <Row gutter={[16, 24]}>
                        <Col span={12}>
                          <ComplianceRing
                            percent={currentOperator.complianceRate}
                            size={100}
                            label="Compliance"
                          />
                        </Col>
                        <Col span={12}>
                          <ComplianceRing percent={currentOperator.riskScore} size={100} label="Risk Score" />
                        </Col>
                      </Row>
                      <Divider />
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                        ...
                        <Tag
                          ...
                          style={{
                            background: currentOperator.bspConnected
                              ? 'rgba(5,150,105,0.08)'
                              : 'rgba(220,38,38,0.08)',
                            color: currentOperator.bspConnected ? '#059669' : '#dc2626',
                            border: 'none',
                            borderRadius: 6,
                            fontWeight: 600,
                            fontSize: 11,
                        ...
                        >
                          {currentOperator.bspConnected ? 'Connected' : 'Disconnected'}
                        </Tag>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                        <Text style={{ fontSize: 12, color: '#64748b' }}>Last Audit</Text>
                        <Text style={{ fontSize: 12, fontWeight: 600 }}>{currentOperator.lastAudit}</Text>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <Text style={{ fontSize: 12, color: '#64748b' }}>Registered</Text>
                        <Text style={{ fontSize: 12, fontWeight: 600 }}>
                          {currentOperator.registrationDate}
                        </Text>
                      </div>
                    </div>
                  </Col>
                  ...
                  <Col xs={24} lg={16}>
                    ...
                    >
                      <Text
                        strong
                        style={{ fontSize: 14, color: '#1e1b4b', display: 'block', marginBottom: 16 }}
                      >
                      ...
                      <Descriptions column={{ xs: 1, sm: 2 }} size="small" bordered>
                        <Descriptions.Item
                          label={
                            <Space>
                              <KeyOutlined style={{ color: '#64748b' }} /> License
                            </Space>
                          }
                        >
                          {currentOperator.licenseNumber}
                        </Descriptions.Item>
                        <Descriptions.Item
                          label={
                            <Space>
                              <CalendarOutlined style={{ color: '#64748b' }} /> Registered
                            </Space>
                          }
                        >
                          {currentOperator.registrationDate}
                        ...
                        <Descriptions.Item
                          label={
                            <Space>
                              <AuditOutlined style={{ color: '#64748b' }} /> Last Audit
                            </Space>
                          }
                        >
                          {currentOperator.lastAudit}
                        </Descriptions.Item>
                        <Descriptions.Item
                          label={
                            <Space>
                              <BankOutlined style={{ color: '#64748b' }} /> BSP Status
                            </Space>
                          }
                        >
                          <Badge
                            status={currentOperator.bspConnected ? 'success' : 'error'}
                            text={currentOperator.bspConnected ? 'Connected' : 'Disconnected'}
                          />
                        </Descriptions.Item>
                        <Descriptions.Item
                          label={
                            <Space>
                              <MailOutlined style={{ color: '#64748b' }} /> Email
                            </Space>
                          }
                        >
                          <a href={`mailto:${currentOperator.email}`}>{currentOperator.email}</a>
                        </Descriptions.Item>
                        <Descriptions.Item
                          label={
                            <Space>
                              <PhoneOutlined style={{ color: '#64748b' }} /> Phone
                            </Space>
                          }
                        >
                        ...
                        <Descriptions.Item
                          label={
                            <Space>
                              <EnvironmentOutlined style={{ color: '#64748b' }} /> Address
                            </Space>
                          }
                          span={2}
                        >
                ...
                <Row gutter={[20, 20]} style={{ paddingBottom: 20 }}>
                  <Col xs={24} lg={16}>
                    ...
                    >
                      ...
                      >
                        <Text strong style={{ fontSize: 14, color: '#1e1b4b' }}>
                          Recent Bookings
                        </Text>
                        ...
                        return {
                          ...
                          children: (
                            ...
                            >
                              ...
                              <Text
                                style={{ fontSize: 13, color: '#1e293b', display: 'block', marginBottom: 6 }}
                              >
                                {audit.notes}
                              </Text>
                              <Text style={{ fontSize: 11, color: '#64748b' }}>Auditor: {audit.auditor}</Text>
                            </div>
                          ),
                        };
                      })}
            ...
            },
            {
              ...
              label: (
                <Space>
                  <TeamOutlined />
                  <span>Agents ({currentOperator.agentCount})</span>
                </Space>
              ),
              children: (
                <div style={{ paddingBottom: 20 }}>
